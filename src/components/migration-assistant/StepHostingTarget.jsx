...
const TARGETS = [
  {
    ...
    label: 'Nginx',
    icon: Server,
    desc: 'Self-hosted VPS. Full control over server config, ideal for custom domains and SSL.',
  },
  {
    id: 'cloudflare',
    label: 'Cloudflare Pages',
    icon: Globe,
    desc: 'Edge-deployed SPA with built-in CI/CD from GitHub. Includes Cloudflare Worker support.',
  },
  {
    id: 'vercel',
    label: 'Vercel',
    icon: Zap,
    desc: 'Instant deploys from GitHub. Best for React SPAs with optional serverless functions.',
  },
  {
    id: 'netlify',
    label: 'Netlify',
    icon: Cloud,
    desc: 'Simple drag-and-drop or Git-based deploys. Built-in redirect rules and env var management.',
  },
];
          ...
          return (
            <button
              ...
              className={`text-left p-6 rounded-xl border-2 transition-all ${
                selected
                  ? 'border-[#73e28a] bg-[#73e28a]/10'
                  : 'border-slate-700 bg-slate-800/50 hover:border-slate-500'
              }`}
            >
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-3 ${
                selected ? 'bg-[#73e28a]/20 text-[#73e28a]' : 'bg-slate-700 text-slate-400'
              }`}>
