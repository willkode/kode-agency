  ...
  const toggle = (label, key, helpText) => (
    <div key={key} className="flex items-start gap-4">
      <button
        ...
        className={`mt-1 w-10 h-6 rounded-full transition-colors flex-shrink-0 relative ${profile[key] ? 'bg-[#73e28a]' : 'bg-slate-600'}`}
      >
        <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-transform ${profile[key] ? 'left-5' : 'left-1'}`} />
      </button>
      <div>
  ...
  return (
    <div>
      ...
      <div className="space-y-5">
        ...
        {field('Base44 API Base URL', 'base44_api_base_url', 'https://api.base44.com/api/apps/YOUR_APP_ID', 'Paste your Base44 backend API URL. Found in your Base44 dashboard under API settings.', true)}
        {field('Frontend Domain (after migration)', 'frontend_domain', 'https://app.yourdomain.com', 'The domain where your self-hosted frontend will live.', true)}
        {field('Base44 App ID', 'base44_app_id', 'abc123xyz', 'Optional. Used in health_ping function template and Worker proxy.')}
        {field('Allowed CORS Origins (comma-separated)', 'cors_origins', 'https://app.yourdomain.com, https://yourdomain.com', 'These will be added to CORS headers in your health endpoint template.')}
        {field('Custom Domain (if any)', 'custom_domain', 'yourdomain.com', 'Used to generate Cloudflare / Vercel domain binding config.')}
